...
def apply_discount(mapping_file, profile, dry_run=True):
        ...
        if 'Classics' in tags:
            ...
            for variant in product['variants']:
                ...
                if variant_format in {'Natural Wood Frame', 'Printed Photo Only', 'Matted'}:
                    price_as_int = mapping[cropping][variant_format][size]
                    compare_at_price = '{:.2f}'.format(price_as_int)
                    discount_price = '{:.2f}'.format(price_as_int * 0.8)
                    ...
                    print(
                        '{title:<30}{cropping:<15}{format:<25}{size:<15}{regular_price:<15}{discount_price:<15}'.format(
                            title=product_title,
                            cropping=cropping,
...
def main():
    apply_discount('prices.yml', 'aitw', dry_run=False)
